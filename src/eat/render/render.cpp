...
void Buffer::resize(size_t n_channels_, size_t n_samples_) {
...
void Buffer::from_interleaved(const InterleavedSampleBlock &b) {
  auto &info = b.info();
...
InterleavedSampleBlock Buffer::to_interleaved(unsigned int sample_rate, size_t start) {
...
void ObjectRenderer::setup_rendering_items(unsigned int fs, const std::vector<std::shared_ptr<ObjectRenderingItem>> &rendering_items,
...
void ObjectRenderer::setup_input_channels(size_t n_in_channels) {
  for (auto &ts : track_specs)
    if (num_tracks_required(ts) > n_in_channels) throw std::runtime_error("more tracks required than given");
}

void ObjectRenderer::process(const float *const *in, float *const *out) {
  // flows:
  // for each object:
...
void DirectSpeakersRenderer::setup_rendering_items(unsigned int fs,
...
void DirectSpeakersRenderer::setup_input_channels(size_t n_in_channels) {
  for (auto &ts : track_specs)
    if (num_tracks_required(ts) > n_in_channels) throw std::runtime_error("more tracks required than given");
}

void DirectSpeakersRenderer::process(const float *const *in, float *const *out) {
...
void HOARenderer::setup_rendering_items(unsigned int fs, const std::vector<std::shared_ptr<HOARenderingItem>> &rendering_items,
                            const channel_map_t &channel_map) {
  n_objects = rendering_items.size();
...
void HOARenderer::setup_input_channels(size_t n_in_channels) {
...
void HOARenderer::process(const float *const *in, float *const *out) {
  zero_samples(out, n_channels, block_size);
...
void CombinedRenderer::setup_input_channels(size_t n_in_channels_) {
...
void CombinedRenderer::setup_rendering_items(unsigned int fs, const std::vector<std::shared_ptr<RenderingItem>> &rendering_items,
                            const channel_map_t &channel_map) {
...
void CombinedRenderer::process(const float *const *in, float *const *out) {
  // flows:
