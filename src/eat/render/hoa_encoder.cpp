...
void write_non_lfe(float *const *out, const float *const *in, size_t n_channels_in,
...
std::vector<double> hoa_gains(double az, double el) {
  std::cout << az << " " << el << std::endl;
  double azr = az * M_PI / 180.0;
  double elr = el * M_PI / 180.0;
  std::vector<double> hoa_gains;
  hoa_gains.push_back(1.0);
  hoa_gains.push_back(sin(azr) * cos(elr));
  hoa_gains.push_back(sin(elr));
  hoa_gains.push_back(cos(azr) * cos(elr));
  hoa_gains.push_back(sqrt(0.75) * sin(2.0 * azr) * cos(elr) * cos(elr));
  hoa_gains.push_back(sqrt(0.75) * sin(azr) * sin(2.0 * elr));
  hoa_gains.push_back(0.5 * (3.0 * sin(elr) * sin(elr) - 1.0));
  hoa_gains.push_back(sqrt(0.75) * cos(azr) * sin(2.0 * elr));
  hoa_gains.push_back(sqrt(0.75) * cos(2.0 * azr) * cos(elr) * cos(elr));
  return hoa_gains;
}

void calculate_hoa_gains(const ear::ObjectsTypeMetadata& metadata,
                         std::vector<float>& direct) {
  auto position = boost::get<ear::PolarPosition>(metadata.position);
  auto gains = hoa_gains(position.azimuth, position.elevation);
  for (size_t i = 0; i < direct.size(); i++) {
    direct[i] = metadata.gain * gains[i];
  }
}
void calculate_hoa_gains(const ear::DirectSpeakersTypeMetadata& metadata,
                         std::vector<float>& direct) {
  auto position = boost::get<ear::PolarSpeakerPosition>(metadata.position);
  auto gains = hoa_gains(position.azimuth, position.elevation);
  for (size_t i = 0; i < direct.size(); i++) {
    direct[i] = gains[i];
  }
...
class ObjectEncoder {
 public:
  ObjectEncoder(size_t hoa_order, size_t block_size_)
      : block_size(block_size_),
        n_channels(hoa_channels[hoa_order]),
        n_channels_out(hoa_channels[hoa_order]),
  ...
  void setup_rendering_items(unsigned int fs, const std::vector<std::shared_ptr<ObjectRenderingItem>> &rendering_items,
    ...
    for (auto &ri : rendering_items) {
      ...
      std::vector<float> direct_gains(n_channels);
      ...
      for (auto &bf : ri->adm_path.audioChannelFormat->getElements<adm::AudioBlockFormatObjects>()) {
        ...
        calculate_hoa_gains(to_otm(*ri, bf), direct_gains);
  ...
  void process(const float *const *in, float *const *out) {
    ...
    write_non_lfe(out, temp_out.ptrs(), n_channels, n_channels_out, block_size);
...
class DirectSpeakersEncoder {
 public:
  DirectSpeakersEncoder(size_t hoa_order, size_t block_size_)
      : block_size(block_size_),
        n_channels(hoa_channels[hoa_order]),
        temp_mono(1, block_size),
        temp(n_channels, block_size) {}
  ...
  void setup_rendering_items(unsigned int fs,
    ...
    for (auto &ri : rendering_items) {
      ...
      for (auto &bf : ri->adm_path.audioChannelFormat->getElements<adm::AudioBlockFormatDirectSpeakers>()) {
        ...
        auto dstm_md = to_dstm(*ri, bf);

        calculate_hoa_gains(to_dstm(*ri, bf), gains);
...
class HOAEncoder {
 public:
  HOAEncoder(size_t hoa_order, size_t block_size_)
      : block_size(block_size_),
        n_channels(hoa_channels[hoa_order]),
        temp_out(n_channels, block_size) {}
  ...
  void setup_rendering_items(unsigned int fs, const std::vector<std::shared_ptr<HOARenderingItem>> &rendering_items,
    ...
    for (auto &ri : rendering_items) {
      ...
      for (auto &type_metadata : ri->type_metadata) {
        if (type_metadata.normalization != "SN3D") {
          throw std::runtime_error(
            "only SN3D supported for input HOA files");
        }

        // Gains set to 1 as we're not changing anything. 
...
class CombinedEncoder {
 public:
  CombinedEncoder(size_t hoa_order, size_t block_size_)
      : n_channels(hoa_channels[hoa_order]),
        block_size(block_size_),
        objects_encoder(hoa_order, block_size),
        direct_speakers_encoder(hoa_order, block_size),
        hoa_encoder(hoa_order, block_size),
        objects_comp_delay(n_channels, objects_encoder.delay()),
        temp1(n_channels, block_size),
        temp2(n_channels, block_size) {}

  void setup_input_channels(size_t n_in_channels_) {
    objects_encoder.setup_input_channels(n_in_channels_);
    direct_speakers_encoder.setup_input_channels(n_in_channels_);
    hoa_encoder.setup_input_channels(n_in_channels_);
  }
  ...
  void setup_rendering_items(unsigned int fs, const std::vector<std::shared_ptr<RenderingItem>> &rendering_items,
    ...
    objects_encoder.setup_rendering_items(fs, objects_items, channel_map);
    direct_speakers_encoder.setup_rendering_items(fs, direct_speakers_items, channel_map);
    hoa_encoder.setup_rendering_items(fs, hoa_items, channel_map);
  }

  size_t delay() { return objects_encoder.delay(); }
  ...
  void process(const float *const *in, float *const *out) {
    ...
    objects_encoder.process(in, temp1.ptrs());
    add_samples(out, temp1.ptrs(), n_channels, block_size);

    direct_speakers_encoder.process(in, temp1.ptrs());

    hoa_encoder.process(in, temp2.ptrs());
    temp1.add(temp2);
 ...
 private:
  ...
  ObjectEncoder objects_encoder;
  DirectSpeakersEncoder direct_speakers_encoder;
  HOAEncoder hoa_encoder;
  ear::dsp::DelayBuffer objects_comp_delay;  // to align non-objects paths
  Buffer temp1;
  Buffer temp2;
};

class HOAEncoderProcess : public StreamingAtomicProcess {
 public:
  HOAEncoderProcess(const std::string &name, const size_t hoa_order_, size_t block_size_,
      ...
      : StreamingAtomicProcess(name),
        ...
        block_size(block_size_),
        hoa_order(hoa_order_),
        n_channels(hoa_channels[hoa_order]),
        encoder(hoa_order, block_size) {
    always_assert(hoa_order <= 2, "Can only handle maximum of 2nd order SN3D");
  }
  ...
  ADMData buildHOAADM() {
    ...
    auto programme = adm::AudioProgramme::create(adm::AudioProgrammeName("HOA Version"));
    auto content = adm::AudioContent::create(adm::AudioContentName("Main"));
    auto object = adm::AudioObject::create(adm::AudioObjectName("Main"));

    adm::AudioPackFormatId ap_id; 
    ap_id.set(adm::TypeDefinition::HOA);
    ap_id.set(adm::AudioPackFormatIdValue(hoa_order));
    auto pack = doc->lookup(ap_id);

    for (size_t i = 0; i < n_channels; i++) {
      adm::AudioChannelFormatId ac_id; 
      ac_id.set(adm::TypeDefinition::HOA);
      ac_id.set(adm::AudioChannelFormatIdValue(i + 1));
      auto channel = doc->lookup(ac_id);
      auto uid = adm::AudioTrackUid::create();
      uid->setReference(pack);
      uid->setReference(channel);
      doc->add(uid);
      object->addReference(uid);
      adm.channel_map[uid->get<adm::AudioTrackUidId>()] = i;
    }
  ...
  void initialise() override {
    ...
    encoder.setup_rendering_items(sample_rate, result.items, adm.channel_map);
  ...
  void process() override {
    while (in_samples->available()) {
      ...
      if (!has_input) {
        ...
        encoder.setup_input_channels(n_input_channels);
        vbs_adapter = std::make_unique<ear::dsp::VariableBlockSizeAdapter>(
            block_size, n_input_channels, n_channels,
            [this](const float *const *in, float *const *out) { encoder.process(in, out); });

        delay_samples = encoder.delay() + static_cast<size_t>(vbs_adapter->get_delay());
        has_input = true;
      } else {
    ...
    // Generate ADM metadata for an HOA-encoded output file.
    auto adm = buildHOAADM();
...
framework::ProcessPtr make_hoa_encoder(const std::string &name, const size_t hoa_order, size_t block_size,
                                  const SelectionOptionsId &options) {
  return std::make_shared<HOAEncoderProcess>(name, hoa_order, block_size, options);
}
}  // namespace eat::render
